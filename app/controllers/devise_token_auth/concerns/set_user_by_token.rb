module DeviseTokenAuth::Concerns::SetUserByToken
  ...
  def set_user_by_token(mapping=nil)
    ...
    if DeviseTokenAuth.enable_standard_devise_support
      ...
      if devise_warden_user && devise_warden_user.tokens.where(client_id: @client_id).last.nil?
  ...
  def is_batch_request?(user, client_id)
    ...
    user.tokens.where(client_id: client_id).last and
    user.tokens.where(client_id: client_id).last.try(:updated_at) and
    Time.parse(user.tokens.where(client_id: client_id).last.updated_at.to_s) > @request_started_at - DeviseTokenAuth.batch_request_buffer_throttle
  end
end
